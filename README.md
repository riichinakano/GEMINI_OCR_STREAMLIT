...
### 2026年4月 — `gemini-2.5-flash` 固定を継続

`gemini-2.5-flash-latest` への変更、および他モデルへの切り替えを検討した結果、**現状維持**とした。

**検討対象モデルと価格（2026年4月時点、100万トークンあたり）**

| モデル | 入力 | 出力 |
|--------|------|------|
| gemini-2.5-flash（採用中） | $0.30 | $2.50 |
| gemini-2.5-flash-lite | $0.10 | $0.40 |
| gemini-2.5-pro | $1.25 | $10.00 |

**判断理由**
- OCR動作に問題なく、精度・安定性ともに十分
- `latest` タグは挙動が予告なく変わるリスクがあるため固定を維持
- `gemini-2.5-flash-lite` はコスト面で魅力的だが、伝票OCRの精度検証が未実施のため見送り
- 将来的に精度比較の上でダウングレードを再検討する余地あり
